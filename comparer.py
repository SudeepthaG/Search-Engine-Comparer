...
def read_files():
    with open("100QueriesSet2.txt", "r") as f:
        queries = f.readlines()
    with open("Google_Result2.json", "r") as f:
...
def write_result(stats, yahoo_results):
    ...
    print('\n\n')
    print('\n\n')
    modified_yahoo_results={}
    i=0
    for key in yahoo_results.keys():
        i=i+1
        modified_yahoo_results['Query '+str(i)]=yahoo_results[key]
    modified_stats = {}
    i = 0
    for key in stats.keys():
        if key=="Averages":
            modified_stats["Averages"]=stats[key]
        else:
            i = i + 1
            modified_stats['Query ' + str(i)] = stats[key]

    with open("hw1.json", "w") as f:
        json.dump(modified_yahoo_results, f)
    with open("hw1.csv", "w") as f:
        f.write("Queries, Number of Overlapping Results, Percent Overlap, Spearman Coefficient\n")
        for query_number, values in modified_stats.items():
            f.write(f"{query_number}, {values['Overlap']}, {values['Percent']}, {values['Rho']}\n")
